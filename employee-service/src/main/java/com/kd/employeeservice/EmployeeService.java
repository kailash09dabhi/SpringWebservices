...
public class EmployeeService {
  ...
  public EmployeeResponse getEmployeeById(Integer id) {
    ...
    employeeResponse.setCurrentCompany(
  ...
  public List<EmployeeResponse> getEmployees() {
    ...
    List<EmployeeResponse> employeeResponseList =
        employees.stream().map(emp -> modelMapper.map(emp, EmployeeResponse.class)).toList();
    employeeResponseList.stream()
        .filter(it -> it.getCurrentCompany() != null)
        .forEach(
            empRes ->
                empRes.setCurrentCompany(
                    restTemplate.getForObject(
                        "/company/{id}",
                        CompanyResponse.class,
                        empRes.getCurrentCompany().getId())));
  ...
  public List<EmployeeResponse> allEmployeesWith(EmployeesRequest employeesRequest) {
    ...
    try {
      if (employeesRequest.getBloodgroup() != null) {
        queryBuilder
            .append("left join company c on e.current_company_id=c.id where e.bloodgroup='")
            .append(employeesRequest.getBloodgroup())
            .append("';");
